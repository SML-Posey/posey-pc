    ...
    py::class_<Command>(m, "Command")
        ...
        .def_property("payload",
            [](Command & data) -> pybind11::array
            {
                ...
                auto base = pybind11::array(dtype, {Command::MaxPayload}, {sizeof(uint8_t)});
                return pybind11::array(
                    dtype, {Command::MaxPayload}, {sizeof(uint8_t)}, data.payload, base);
            },
            [](Command & data, pybind11::array_t<uint8_t> new_data)
            {
                if (new_data.size() >= Command::MaxPayload)
                    throw std::invalid_argument(fmt::format("Must set to max {:d} element vector!", Command::MaxPayload));
