...
void add_support(Tm& c) {
    c.def(
         "add_listener",
         [](PyMessageListener& ml, typename Td::Buffer& buffer) {
             return ml.add_listener(
                 Td::message_id, buffer.capacity(), &buffer.copy_callback);
         })
        .def(
            "add_listener",
            [](PyMessageListener& ml, BufferMessagePair<Td>& message) {
                return ml.add_listener(message);
            })
        .def(
            "write",
            [](PyMessageListener& ml, const typename Td::Buffer& buffer) {
                return ml.write(buffer.get_buffer(), buffer.used());
            })
        .def(
            "write",
            [](PyMessageListener& ml, const BufferMessagePair<Td>& message) {
                return ml.write(
                    message.buffer.get_buffer(), message.buffer.used());
            });
}

void platform_io_MessageListener(py::module& m) {
    auto c =
        py::class_<PyMessageListener>(m, "MessageListener")
            .def(py::init<>())
            .def("listener_index", &PyMessageListener::listener_index)
            .def_property_readonly(
                "num_messages", &PyMessageListener::num_messages)
            .def_property_readonly(
                "num_ignored_messages",
                &PyMessageListener::num_ignored_messages)
            .def_property_readonly(
                "num_skipped_bytes", &PyMessageListener::num_skipped_bytes)
            ...
            .def(
                "write",
                [](PyMessageListener& ml, const py::bytes& buffer) {
            ...
            .def(
                "read",
                [](PyMessageListener& ml, uint16_t length) {
                    if (length > ml.used())
                        length = ml.used();
